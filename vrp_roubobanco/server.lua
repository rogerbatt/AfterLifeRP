...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbankrobbery = "https://discordapp.com/api/webhooks/743959233501855844/FQhjhFnpAJFFUWq24TRF0ZZSxHYsYk7tcd5mEmbERS5boWuOLMTr1qcssQiBGA_PizGx"
--------------------------------------------------------------------------------------------------------------------------------------------
--CHAMADO
--------------------------------------------------------------------------------------------------------------------------------------------
...
function vRPrb.updateCops(message)
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        TriggerClientEvent('chatMessage', -1, "[AFTER LIFE BANCO CENTRAL]", {64,64,255}, message)
    end
end
...
RegisterServerEvent('vrp_roubobanco:lotSf')
AddEventHandler('vrp_roubobanco:lotSf', function(bank, safe)
    local src = source
	local xPlayer = vRP.getUserId(src)
